...
export function Sidebar({ activeView, onNavigate, onLogout, collapsed, onToggle }: SidebarProps) {
  return (
    <>
      ...
      <aside
        className={`fixed top-0 left-0 z-30 h-full bg-white dark:bg-zinc-900 border-r border-zinc-200 dark:border-zinc-800 transition-all duration-200 flex flex-col ${
          collapsed ? "w-16" : "w-64"
        }`}
      >
        <div className="h-14 flex items-center gap-2 px-4 border-b border-zinc-200 dark:border-zinc-800">
          ...
          {!collapsed && (
            <span className="font-semibold text-sm tracking-wide">
              NOVARA <span className="text-zinc-500 dark:text-zinc-400 font-normal">Control</span>
            </span>
          )}
        </div>
            ...
            >
              ...
              {!collapsed && (
                <>
                  ...
                  {item.badge && (
                    <span className="ml-auto text-xs bg-zinc-100 dark:bg-zinc-800 text-zinc-700 dark:text-zinc-300 px-1.5 py-0.5 rounded-full">
                      {item.badge}
                    </span>
                  )}
                </>
        ...
        <div className="p-2 border-t border-zinc-200 dark:border-zinc-800">
          <button
            onClick={onToggle}
            className="w-full flex items-center gap-3 px-3 py-2 rounded-lg text-sm text-zinc-500 hover:text-zinc-700 dark:hover:text-zinc-300 hover:bg-zinc-100 dark:hover:bg-zinc-800 transition-colors"
          ...
          <button
            onClick={onLogout}
            className="w-full flex items-center gap-3 px-3 py-2 rounded-lg text-sm text-zinc-500 dark:text-zinc-400 hover:text-red-400 hover:bg-zinc-100 dark:hover:bg-zinc-800 transition-colors"
          >
            <LogOut size={18} />
