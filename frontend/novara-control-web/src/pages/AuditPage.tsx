  ...
  return (
    <div className="p-6 space-y-4">
      <div className="flex items-center gap-2">
        <ScrollText size={18} className="text-zinc-500 dark:text-zinc-400" />
        ...
        <div className="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-xl overflow-hidden">
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody>
                {(logs ?? []).map((log) => (
                  <tr key={log.id} className="border-b border-zinc-200/50 dark:border-zinc-200 dark:border-zinc-800/50">
                    <td className="px-4 py-3 text-zinc-500 text-xs whitespace-nowrap">{log.created_at}</td>
                    <td className="px-4 py-3 text-zinc-300 dark:text-zinc-700 dark:text-zinc-300">{log.admin_name}</td>
                    <td className="px-4 py-3">
                      <code className="text-xs bg-zinc-100 dark:bg-zinc-800 px-1.5 py-0.5 rounded text-zinc-300 dark:text-zinc-700 dark:text-zinc-300">{log.action}</code>
                    </td>
                    <td className="px-4 py-3 text-zinc-500 dark:text-zinc-400 text-xs hidden md:table-cell">
                      {log.target_type} {log.target_id ? `#${log.target_id}` : ""}
                      {log.school_name && <span className="text-zinc-400 dark:text-zinc-600"> &middot; {log.school_name}</span>}
                    </td>
                ...
                {(logs ?? []).length === 0 && (
                  <tr>
                    <td colSpan={5} className="px-4 py-8 text-center text-zinc-400 dark:text-zinc-600 text-sm">No audit logs</td>
                  </tr>
                )}
              </tbody>
            </table>
